"""
strategy/regime.py — daily-timeframe regime gate for MCX commodity momentum
strategies.
...
backtest_commodity.py's ENTRY_THRESHOLDS["crude"] comment for the full
...
def regime_ok(daily_closes: list[float], window: int = 15, min_autocorr: float = 0.0) -> bool | None:
    ...
    # with `is False`/`is True` (see backtest_commodity.py, entry_signal.py)
