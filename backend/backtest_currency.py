...
Mirrors backtest_commodity.py's proven structure exactly (same entry rules,
...
Differences from the commodity path, both real and deliberate:
  - Cost model: strategy/currency_costs.py (NO STT/CTT on currency
...
Usage:
    python3 backtest_currency.py --symbols USDINR
    python3 backtest_currency.py --symbols USDINR EURINR GBPINR JPYINR
"""
...
from strategy.currency_costs import compute_ncd_currency_costs, size_currency_lots
from strategy.commodity_features import compute_commodity_features

ARCHIVE_DIR = Path(__file__).resolve().parent / "archive_currency"
