...
Rebuilt 2026-09-19 (see strategy/equity_universe.py's docstring for the full
...
Mirrors backtest_commodity.py/backtest_currency.py's proven walk-forward
...
calibration. Two reasons:
  1. The original removed version tuned/curated per-stock, which is exactly
     what made its universe selection circular (see strategy/equity_universe.py).
...
Every other backtest in this codebase (backtest_commodity.py,
backtest_currency.py) processes at most a handful of symbols and runs each
...
0.50), same honest-first-baseline posture backtest_currency.py took before
any threshold tuning happened.

Usage:
    python3 backtest_equity.py --symbols RELIANCE TCS
    python3 backtest_equity.py                          # full NIFTY50 (49 names) universe
"""
...
from strategy.equity_costs import compute_nse_equity_costs, size_equity_shares
