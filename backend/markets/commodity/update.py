#!/usr/bin/env python3
"""
markets/commodity/update.py — CLI: Top up MCX Commodity Archives (REAL data via Upstox) & Retrain ML Models

Usage:
    python3 -m markets.commodity.update           # Update archives + retrain models
    python3 -m markets.commodity.update --no-train # Update archives only
...
for that. Archives are fetched via markets/commodity/data.py (genuine Upstox
history) -- this used to call markets/commodity/synthetic_data.py's synthetic
...
from markets.commodity.data import build_all_real_commodity_archives
